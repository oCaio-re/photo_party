...
export function EventChatView({
  ...
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col h-[76vh] sm:h-[80vh] bg-[#fffaf5] border border-[#cb7d87]/25 rounded-3xl shadow-sm overflow-hidden">
      {/* Header Bar */}
      <div className="px-4 py-3 bg-[#fffaf5] border-b border-[#cb7d87]/15 flex items-center justify-between gap-2 shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-[#cb7d87]/15 flex items-center justify-center text-[#cb7d87]">
            <MessageSquare className="w-4 h-4" />
          </div>
          <div>
            <h3 className="font-serif text-base text-[#5a6248] font-medium leading-none">
              Mural de Recados
            </h3>
        ...
        <div className="flex items-center gap-2">
          {onChangeName && (
            <button
              ...
              className="flex items-center gap-1 text-[11px] text-[#5a6248] hover:text-[#cb7d87] bg-white border border-[#cb7d87]/20 px-2.5 py-1 rounded-full transition-colors cursor-pointer"
          ...
          <button
            ...
            className="p-1.5 text-[#5a6248] hover:text-[#cb7d87] rounded-full transition-colors cursor-pointer"
      ...
      <div className="px-3 py-2 bg-[#fbead6]/40 border-b border-[#cb7d87]/10 flex items-center gap-1.5 overflow-x-auto scrollbar-none shrink-0">
        <span className="text-[11px] font-semibold text-[#832d3b] whitespace-nowrap pl-1 flex items-center gap-1">
        ...
        {CHEER_PROMPTS.map((prompt) => (
          <button
            ...
            disabled={isSending}
            className="text-[11px] px-2.5 py-1 rounded-full bg-white border border-[#cb7d87]/25 text-[#5a6248] hover:bg-[#cb7d87]/15 hover:text-[#832d3b] transition-colors whitespace-nowrap cursor-pointer shadow-2xs"
          >
            {prompt}
          </button>
        ))}
      ...
      <div
        ref={containerRef}
        className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#fffaf5]/50"
      >
        {isLoading ? (
          <div className="h-full flex items-center justify-center text-[#7c8764]">
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-6">
            <div className="w-12 h-12 rounded-full bg-[#cb7d87]/10 flex items-center justify-center text-[#cb7d87] mb-2">
              <Sparkles className="w-6 h-6" />
            </div>
            <h4 className="font-serif text-lg text-[#5a6248]">Nenhum recado ainda</h4>
            <p className="text-xs text-[#7c8764] mt-1 max-w-xs">
                ...
                <div className="flex items-center gap-1.5 mb-1 px-1 text-[11px]">
                  <span
                    className={`font-medium ${
                      isMe ? "text-[#cb7d87] font-semibold" : "text-[#5a6248]"
                    }`}
                  >
                    {isMe ? "Você" : msg.guestName}
                  </span>
                  {msg.tableIdentifier && (
                    <span className="bg-[#5a6248]/15 text-[#5a6248] text-[9px] px-1.5 py-0.2 rounded-full font-medium">
                      {msg.tableIdentifier}
                    </span>
                  )}
                  <span className="text-[#7c8764]/75 text-[10px]">
                ...
                <div className="relative flex items-center gap-2 max-w-[85%] sm:max-w-[75%]">
                  {isMe && (
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 transition-opacity p-1 text-rose-400 hover:text-rose-600 rounded-full hover:bg-rose-50 cursor-pointer"
                  ...
                  <div
                    className={`px-3.5 py-2 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-2xs break-words ${
                      isMe
                        ...
                        : "bg-white text-[#3f4633] border border-[#cb7d87]/20 rounded-tl-xs"
                    }`}
                  >
                    {msg.message}
                  </div>
      ...
      <form
        ...
        className="p-3 bg-white border-t border-[#cb7d87]/15 flex items-center gap-2 shrink-0"
      >
        <input
          type="text"
          value={inputText}
          ...
          maxLength={500}
          className="flex-1 px-4 py-2.5 rounded-full border border-[#cb7d87]/30 bg-[#fffaf5] text-xs sm:text-sm text-[#5a6248] placeholder-[#7c8764]/60 focus:outline-hidden focus:ring-2 focus:ring-[#cb7d87]"
        />
